class PKManager {
    ...
    public onPK(type,data){
        ...
        if(!data.dealAward && type != PKManager.PKType.REPLAY)
        {
            ...
            if(award.card)
                this.pkAward.prop.push({type:'card',des:'×' + award.card})
            if(award.diamond)
                this.pkAward.prop.push({type:'diamond',des:'×' + award.diamond})
            if(award.energy)
                this.pkAward.prop.push({type:'energy',des:'×' + award.energy})
            //if(award.collect)
            //    this.pkAward.prop.push({type:'card',des:'×' + award.collect})
            if(award.g_exp)
            {
