class HelpManager {
    ...
    public constructor() {
        ...
        this.helpObj['day'] = {
            title:'究极研究院说明',
            list:[
                '究极研究院的老怪物们醉心于卡兵的终极力量，试图开发出每个卡兵以弱胜强的能力',
        ...
        this.helpObj['map'] = {
            ...
            list:[
                ...
                '玩家手下的卡兵会在野外不断地进行扫荡，当卡兵的实力得到提升后，扫荡效率会得到相应的提高',
                '卡兵在扫荡野外据点时，起作用的只是三围属性（攻，血，速），不会触发卡兵技能',
                '高级据点的敌人会更强，但消灭后得到的功勋也会越多',
                '在野外扫荡时，每消灭一个敌人，都会获得一定的功勋并得到一张该据点的通辑令。功勋会收集到功勋背包中，需要玩家点击获取才会放进功勋仓库。',
                '如果功勋背包已满，消灭敌人时将不会得到功勋，但通辑令仍在继续收集',
                '玩家可以使用功勋仓库中的功勋兑换金币和卡兵碎片',
                '玩家如果要进入高一级据点，需完成前一据点的挑战任务：消灭指定数量的通辑令BOSS',
                '玩家使用通辑令可搜寻到一组敌人，玩家可花费[1点]体力进行挑战，也可使用[1张]通辑令搜寻新的敌人',
                '即使玩家进入了更高级的据点，还是可以回到前[2]个据点进行扫荡的，但切换据点时，当前据点的通辑令会被丢弃'
            ]
        }
        ...
        this.helpObj['pk'] = {
            title:'PK说明',
            list:[
                '玩家每次PK时，可以从系统给出的8位卡兵中选出最多6个进行出战',
                '玩家布阵时共拥有88个符文，每个卡兵需花费一定数量的符文才能召唤出阵',
                '每个卡兵不限出战次数，但重复上阵时，召唤所需要的符文会更多',
                ...
                '只有出战单位才能使用绝招，而绝招需要满怒气才能释放。每个卡兵需要的怒气上限不同，可在卡兵详情页中进行查看',
        ...
        this.helpObj['tec'] = {
            title:'升级说明',
            list:[
                '当收集到足够的卡兵碎片和金币后，玩家就可以对该卡兵进成升级',
                '升级卡兵后，该卡兵的单兵战力就会得到极大提升。同时，玩家的总战力也会得到相应的提升',
                '由于卡兵的出战战力是由玩家战力和卡兵单兵战力共同构成的，所以每次升级卡兵，其它卡兵的出战战力也会有所提升，只是被升级的那个卡兵能得到更多的提升'
            ]
        }
        ...
        this.helpObj['pve'] = {
            ...
            list:[
                ...
                '在' + name + '中，不同难度对应的战力和卡兵等级上限是不一样的，如果玩家属性高于难度要求，则会把玩家属性[降至对应值]进行PK',
