class HelpManager {
    ...
    public constructor() {
        this.helpObj['main'] = {
            ...
            list:[
                ...
                '卡士公会的[精英挑战]会有[属性限制]，但挑战胜利后会获得丰富的[战力]和[金币]奖励',
        ...
        this.helpObj['day'] = {
            title:'每日任务说明',
            list:[
                '每日任务每天会推出[10个]关卡让玩家进行挑战，[难度]和[奖励]会逐渐[递增]',
                '在每日任务中，每次挑战都需花费[1点]体力',
                '玩家在每日任务的挑战中能获得[大量奖励]，[性价比]远高于其它PK玩法',
                '在[同一服务器]的所有玩家，每日任务中推出的手牌和所遇到的关卡是[一样]的，跟同服的其它玩家交流通关心得会让你在通关的过程中少走弯路',
                '每日任务中玩家在偶数关卡取得胜利，就会获得[1点]战力奖励',
                '每日任务挑战中玩家使用的是[修正场规则]（玩家的手牌战力固定为1000，与玩家的实际战力[无关]）'
            ]
        }
        this.helpObj['server'] = {
            title:'天梯竞技场说明',
        ...
        this.helpObj['pk'] = {
            title:'PK说明',
            list:[
                '玩家每次PK时，可以从系统给出的[8(6)位]卡兵中选出最多[6个]进行出战',
        ...
        this.helpObj['leader_skill'] = {
            title:'队伍技能说明',
            list:[
                '队伍技能可在[命运之门]开启中获得',
                '玩家可在[布阵]中决定使用哪个队伍技能或[不使用]队伍技能',
                '队伍技能会在战斗的[过程中]生效，具体效果可看技能介绍',
                '每个队伍技能都有着[数量限制]，在[单个服务器]中，如果该技能拥有者[已达上限]，将[不会]再出现被其它玩家所得到，具体拥有者信息可在[图鉴]中看到',
                '服务器中会[定期]推出新的队伍技能，[最近一个]新的技能的出现时间可在[图鉴]中看到',
                '队伍技能,在[修正场规则]中[不起作用]'
            ]
        }

        this.helpObj['leader_draw'] = {
            title:'命运之门说明',
            list:[
                '玩家可使用[命运石]来[开启]命运之门',
                '如果命运石[不足]，玩家可用钻石代替，[100钻石=1命运石]',
                '玩家可使用[10]个[命运石碎片]合成[1]个命运石',
                '在命运之门中有机率得到[队伍技能]'
            ]
        }
        ...
        this.helpObj['guess'] = {
            title:'竞猜说明',
            list:[
                '在竞猜系统中，玩家需从[两组]卡兵队伍中，选择你认为[胜利]的那一组进行下注',
                '在竞猜系统中，玩家竞猜成功后会得到[竞猜奖励]，猜错[没有]损失',
                '在竞猜系统中，玩家每天可进行多次竞猜，竞猜[次数]与[玩家等级]挂钩，[跨天]次数会被[重置]'
            ]
        }
    ...
    public leaderSkillHelp(){
        HelpUI.getInstance().show(this.helpObj['leader_skill']);
    }
    public leaderDrawHelp(){
        HelpUI.getInstance().show(this.helpObj['leader_draw']);
    }
}